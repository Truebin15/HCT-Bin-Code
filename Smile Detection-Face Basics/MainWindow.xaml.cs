...
namespace Microsoft.Samples.Kinect.FaceBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        {
            //Create log file and record the system start time
            double startTime = DateTime.Now.Subtract(DateTime.Parse("1970-1-1")).TotalMilliseconds;
            this.fileName = "FaceBasics-log" + startTime.ToString() + ".csv";
            this.startDay = Int32.Parse(DateTime.Now.ToString("dd"));
            this.lastRefreshTime = startTime;

            using (StreamWriter sw = new StreamWriter(this.fileName))
            {
                string entries = "UserID,HeadBoxLeft,HeadBoxRight,HeadBoxTop,HeadBoxBottom," +
                                 "PointX1,PointY1,PointX2,PointY2,PointX3,PointY3,PointX4,PointY4," +
                                 "PointX5,PointY5,Happy,Engaged,WearingGlasses,LeftEyeClosed," +
                                 "RightEyeClosed,MouthOpen,MouthMoved,LookingAway,FaceYaw,FacePitch," +
                                 "FacenRoll,Date,Time\n";
                sw.Write(entries);
            }
        }
        ...
        private void DrawFaceFrameResults(int faceIndex, FaceFrameResult faceResult, DrawingContext drawingContext)
        {
            ...
            string faceText2 = string.Empty;
            // Record the UserID*************
            faceText += faceIndex.ToString() + ",";
            faceText2 += "UserID : " + faceIndex.ToString() + "\n";
            ...
            // Record head box
            faceText += faceBoxSource.Left.ToString() + "," +
                        faceBoxSource.Right.ToString() + "," +
                        faceBoxSource.Top.ToString() + "," +
                        faceBoxSource.Bottom.ToString() + ",";
            ...
            if (faceResult.FacePointsInColorSpace != null)
            {
                ...
                foreach (PointF pointF in faceResult.FacePointsInColorSpace.Values)
                {
                    ...
                    // Record the coordinates of eyes etc.
                    faceText += pointF.X + "," +
                                pointF.Y + ",";
                    i += 1;
                }
            ...
            if (faceResult.FaceProperties != null)
            {
                foreach (var item in faceResult.FaceProperties)
                {
                    ...
                    if (item.Value == DetectionResult.Maybe)
                    {
                        ...
                        faceText2 += DetectionResult.No + "\n";
                    }
                    else
                    {
                        faceText += item.Value.ToString() + ",";
                        faceText2 += item.Value.ToString() + "\n";
                    }                    
                }
            }
            ...
            // Record timestamp and id of users******************
            faceText += DateTime.Now.ToString("yyyy-MM-dd") + "," +
                        DateTime.Now.ToString("HH:mm:ss.fff") + "\n";

            //Determine whether to create a new log file*************
            int currentDay = Int32.Parse(DateTime.Now.ToString("dd"));
            int dayPassed = currentDay - this.startDay; 
            
            //One log per day
            if (dayPassed != 0)
            {
                //Create a new log file and update the startDay
                this.CreateLogFile();
            }

            // Determine whether to refresh the log file******************
            double currentTime = DateTime.Now.Subtract(DateTime.Parse("1970-1-1")).TotalMilliseconds;
            double timePassed = currentTime - this.lastRefreshTime;

            // About 25 records per second***********
            if (timePassed > 32)
            {
                //Add the information into the log file*****************
